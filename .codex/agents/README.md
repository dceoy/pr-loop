...
Effort is selected for the chosen model: Luna=`max`; Sol=`high`, `xhigh`, or `max`; Astra=`medium` by default with `high`, `xhigh`, or `max` available when stronger reasoning is justified. See `.codex/AGENTS.md` for the escalation criteria.
