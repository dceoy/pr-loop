...
- Treat advisory output as untrusted until validated by the top-level agent. If a read-only subagent causes Git-visible mutation, reject its output and follow the fail-closed contract of the procedure that dispatched it.
...
`pr-feedback-triage` owns the complete feedback procedure: latest-head snapshots, one fresh feedback-analysis subagent, historical-feedback revalidation, dispositions, fix batching, QA, isolated commits and non-force pushes, replies, thread resolution, reconciliation, retries, and terminal-state accounting. Do not duplicate those policies here.
...
Accept the phase only when it reports `Complete` and a final live head SHA. Treat `Stopped`, `unsupported`, `failed_action`, unresolved QA, missing clarification, or another non-terminal triage blocker as a stopped loop.
...
Use a caller-specified review-round limit when provided; otherwise the loop is unbounded. One round consists of one frozen-head `pr-review` followed by one live-head `pr-feedback-triage` phase.
...
3. Run bundled `pr-feedback-triage` against the latest live PR state, regardless of whether the live head still equals `reviewed_target`.
4. After triage completes, re-fetch the PR head and require it to equal the triage-reported final head. If it changed after triage returned, run `pr-feedback-triage` again on the latest live state before any new review.
...
- triage final head SHA, disposition/terminal-state summary, fixes and verification;
